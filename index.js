...
async function startXeonBotInc() {
    ...
    if (pairingCode && !XeonBotInc.authState.creds.registered) {
        ...
        if (!!global.phoneNumber) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFormat: 2637810626048 (without + or spaces) : `)))
        }
        ...
        if (!pn('+' + phoneNumber).isValid()) {
            console.log(chalk.red('Invalid phone number. Please enter your full international number (e.g., 151234567 for US, 447911123456 for UK, etc.) without + or spaces.'));
            process.exit(1);
        }
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.magenta(`${global.themeemoji || '•'} GITHUB: TKT-TECH`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} WA NUMBER: ${owner}`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} CREDIT: TAFADZWA-TKT`))
